...
```bash
# Raw JSON (for capturing TMDB IDs programmatically)
...
```bash
bash skills/radarr/scripts/radarr add <tmdbId>                        # Searches immediately (default)
bash skills/radarr/scripts/radarr add <tmdbId> <profileId>            # Use specific quality profile
bash skills/radarr/scripts/radarr add <tmdbId> --no-search            # Add without searching
```
...
```bash
bash skills/radarr/scripts/radarr remove <tmdbId>                # Keep files
bash skills/radarr/scripts/radarr remove <tmdbId> --delete-files # Delete files too
```
