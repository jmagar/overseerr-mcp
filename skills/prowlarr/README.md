...
Add the following to `~/.config/overseerr-mcp/.env`:
...
The skill loads credentials from `~/.config/overseerr-mcp/.env`. You can also override them temporarily:
...
→ Check your `.env` file exists at `~/.config/overseerr-mcp/.env` and contains `PROWLARR_URL` and `PROWLARR_API_KEY`
