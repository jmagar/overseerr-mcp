...
Add these variables to `~/.config/overseerr-mcp/.env`:
...
- Credentials are loaded from `~/.config/overseerr-mcp/.env`
...
→ Create `.env` file at `~/.config/overseerr-mcp/.env`

**"QBITTORRENT_URL and QBITTORRENT_USERNAME and QBITTORRENT_PASSWORD must be set in .env"**
→ Check that all three variables are defined in `~/.config/overseerr-mcp/.env`
