...
Add these lines to `~/.config/overseerr-mcp/.env`:
...
The scripts load credentials from `~/.config/overseerr-mcp/.env`:
...
→ Check that `SABNZBD_URL` and `SABNZBD_API_KEY` are set in `~/.config/overseerr-mcp/.env`
