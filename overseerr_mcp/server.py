...
overseerr_client: Optional[OverseerrApiClient] = None
try:
    ...
    if isinstance(api_response, dict) and "results" in api_response:
        for item in api_response["results"]:
            ...
            results.append({
                ...
                "year": (item.get("releaseDate", "") or item.get("firstAirDate", "") or "").split("-")[0] or None,
...
async def request_movie(ctx: Context, tmdb_id: int) -> Union[Dict, str]:
    ...
    api_response = await client.post("/request", json_data={"mediaType": "movie", "mediaId": tmdb_id})
    if isinstance(api_response, str):
        return api_response
...
async def request_tv_show(ctx: Context, tmdb_id: int, seasons: Optional[List[int]] = None) -> Union[Dict, str]:
    ...
    payload: Dict[str, Any] = {"mediaType": "tv", "mediaId": tmdb_id, "seasons": seasons if seasons else "all"}
    ...
    if (
        ...
        logger.info(f"TV show TMDB ID {tmdb_id} requested (seasons: {payload['seasons']}). Request ID: {api_response['id']}")
        return api_response
...
async def list_failed_requests(ctx: Context, count: int = 10, skip: int = 0) -> Union[List[Dict], str]:
    ...
    api_response = await client.get("/request", params={"take": count, "skip": skip, "sort": "modified", "filter": "failed"})
    if isinstance(api_response, str):
        return api_response
...
async def health_check(request: Request) -> JSONResponse:
    ...
    if not OVERSEERR_URL or not OVERSEERR_API_KEY:
        return JSONResponse(
            {"status": "error", "service_accessible": False, "reason": "OVERSEERR_URL or OVERSEERR_API_KEY not configured"},
            status_code=500,
        )

    if not client:
        return JSONResponse(
            {"status": "error", "service_accessible": False, "reason": "Overseerr client not initialized"},
            status_code=503,
        )
...
def main() -> None:
    ...
    logger.info(f"Starting Overseerr MCP Server — transport: {OVERSEERR_MCP_TRANSPORT}, port: {OVERSEERR_MCP_PORT}")

    if OVERSEERR_MCP_TRANSPORT == "stdio":
        mcp.run()
    elif OVERSEERR_MCP_TRANSPORT in ("http", "streamable-http"):
        mcp.run(transport="http", host=OVERSEERR_MCP_HOST, port=OVERSEERR_MCP_PORT, path="/mcp")
